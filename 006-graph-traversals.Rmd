...
Ensure that the several packages are loaded
```{r, results=FALSE}
library(DiagrammeR)
library(magrittr)
```
...
Now that there is a property graph, we can find out bits of information without directly inspecting such information. This will be important when a property graph becomes quite large since manually inspecting the data gets difficult and impractical.
...
The `rescale_edge_attrs()` also works to scale across two colors. Here we take the `commits` edge attribute as before but scale from the color `gray95` (light gray) to the color `gray5` (dark gray). The color values (represented as hexadecimal color codes) are placed in the `color` edge attribute column. Here, we'll transform the graph and also display the edf to show the values in the new `color` column. 
...
get_highest_committer <- function(graph, project_name) {
  ...
  graph %>% 
    select_nodes("project", project_name) %>%
    trav_in_edge %>%
    cache_edge_attrs_ws("commits", "numeric") %>%
    trav_in_node %>%
    trav_in_edge("commits", max(get_cache(.))) %>%
...
# Call the function for each of the repositories
...
Graph traversals are undoubtedly useful operations. The nomenclature can be a point of confusion at first (I admit that their function names require some thought as to what traversal will result). However, given time and some practice, traversals will enable you to gleaning the appropriate information from a graph with the greatest of ease. As a graph becomes ever more complicated with the additions of nodes and edges, traversal functions (coupled with knowledge of a graph's underlying data model) show their utility and worth.

...
Perhaps you don't work directly with the node IDs and instead with unique node labels. This is a common practice as node ID values can be considered as arbitrary but node labels and other attributes give each node an identity and make them distinguishable. In such a workflow, it's easier to create edges based on the node `label` values. Supply the node labels as values for the `from` and `to` arguments and set `use_labels` to `TRUE`. To view the graph's edges use `get_edges()` as before but, this time, use `return_values = "label"` to observe the graph's edges in terms of node `label` values.
...
The `get_edges()` function can output the pairs of nodes in edges either as a character vector (as above, it's the default), as a data frame (with 2 columns: `from` and `to`), or as a list (first component is the `from` vector and the second represents the `to` nodes). Here are examples of the latter two output types:

```{r get_edges_return_type_df}
# Get the graph's edges as a data frame
get_edges(graph_edge_w_labels, return_type = "df")
```

```{r get_edges_return_type_list}
# Get the graph's edges as a list
get_edges(graph_edge_w_labels, return_type = "list")
```

The addition of a node and the creation of edgeas can also be performed in a single `add_node()` step. You can use either (or both) of the optional `from` and `to` arguments in the `add_node()` function. This is best demonstrated through several examples.
...
render_graph(graph_e, output = "visNetwork")
```
...
edges_before_change <- edge_count(graph_f)

# Remove node `6` from the graph
graph_g <- delete_node(graph_f, node = 6)

# Get the number of edges after the removal
edges_after_change <- edge_count(graph_g)
...
render_graph(graph_g, output = "visNetwork")
```
...
graph_h <- delete_edge(graph_g, from = 5, to = 2)
...
render_graph(graph_h, output = "visNetwork")
```
